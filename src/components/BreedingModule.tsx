...
export const BreedingModule: React.FC<BreedingModuleProps> = ({ 
  ...
  // FIX: Using original 'M' and 'H' from v3.0 types
  const males = breeders.filter(b => b.sex === 'M' && b.status === 'active');
  const females = breeders.filter(b => b.sex === 'H' && b.status === 'active');
