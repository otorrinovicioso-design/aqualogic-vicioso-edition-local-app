  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h3 className="text-2xl font-display font-bold text-white">Reproductores Elite</h3>
          <p className="text-[10px] text-slate-500 uppercase tracking-widest">Seguimiento Individual de Ejemplares</p>
        </div>
        <Button 
      ...
      {showForm && (
        <Card className="animate-in fade-in slide-in-from-top-4 duration-300">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" className="w-full">Registrar Reproductor</Button>
          </form>
        </Card>
      )}
      ...
      <div className="grid gap-4">
        {breeders.length === 0 ? (
          <p className="text-center text-slate-500 py-10 italic">No hay reproductores registrados individualmente.</p>
        ) : (
          breeders.map((breeder) => (
            <Card key={breeder.id} className="relative overflow-hidden group">
              <div className="flex gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start">
                    <div>
                      <h4 className="font-bold text-white text-lg">{breeder.name}</h4>
                      <p className="text-[10px] text-slate-500 uppercase tracking-widest">{breeder.species} • {breeder.sex}</p>
                    </div>
                    <button 
