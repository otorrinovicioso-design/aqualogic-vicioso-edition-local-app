  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="grid grid-cols-2 gap-4">
        ...
        <Card className={cn("p-4 h-32 flex flex-col justify-between border transition-all", activeIncidents.length > 0 ? "bg-red-500/10 border-red-500/20 animate-pulse" : "bg-white/5 border-white/10")}>
           <div className="flex items-center gap-3"><div className={cn("w-8 h-8 rounded-lg flex items-center justify-center font-bold", activeIncidents.length > 0 ? "bg-red-500 text-slate-950" : "bg-white/10 text-slate-700")}><AlertTriangle size={20} /></div><span className={cn("text-[10px] font-bold uppercase tracking-widest", activeIncidents.length > 0 ? "text-red-500" : "text-white/50")}>ALERTAS</span></div>
           <div className="flex items-end justify-between"><div className="flex flex-col"><span className="text-3xl font-display font-bold text-white">{activeIncidents.length}</span><span className="text-[9px] text-slate-700 font-bold uppercase tracking-widest">CASOS</span></div>{activeIncidents.length === 0 && <span className="text-[10px] text-emerald-500 font-bold italic tracking-widest">OK</span>}</div>
        </Card>
      </div>

      <Card className="p-5 bg-white/5 border border-white/10">
        <div className="flex items-center gap-2 mb-6 border-b border-white/5 pb-2 text-white font-white uppercase"><Layers size={18} className="text-gold-400" /><h3 className="text-xs font-black tracking-widest">Inventario</h3></div>
        ...
        <div className="mt-6 pt-4 border-t border-white/5">
           {filteredCensus.filter(c => c.type === 'Alevines').map(item => (
             <div key={item.id} className="flex justify-between items-center bg-aqua-400/5 p-3 rounded-xl border border-aqua-400/10">
      ...
      <Card className="p-5 bg-white/5 border border-white/5 opacity-80">
        {latestParams ? (
          <div className="grid grid-cols-4 sm:grid-cols-7 gap-4">
            {[ {l:'pH',v:latestParams.ph,c:'text-aqua-400'}, {l:'T°',v:latestParams.temp,c:'text-white font-white'}, {l:'gH',v:latestParams.gh,c:'text-slate-400'}, {l:'kH',v:latestParams.kh,c:'text-slate-400'}, {l:'NH3',v:latestParams.nh3,c:'text-orange-500'}, {l:'NO2',v:latestParams.no2,c:'text-red-500'}, {l:'NO3',v:latestParams.no3,c:'text-orange-400'} ].map(m => (
              <div key={m.l} className="text-center bg-slate-900/50 p-2 rounded-xl border border-white/5"><p className="text-[7px] text-slate-700 uppercase font-black mb-1">{m.l}</p><p className={cn("text-xs font-bold", m.c)}>{m.v || '-'}</p></div>
            ))}
          </div>
