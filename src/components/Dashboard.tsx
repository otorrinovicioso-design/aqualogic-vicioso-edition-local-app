...
interface DashboardProps {
  ...
  healthRecords: HealthRecord[]; // Añadido para alertas de enfermos
  incidents: Incident[];
...
export const Dashboard: React.FC<DashboardProps> = ({ censusData = [], waterParams = [], healthRecords = [], incidents = [], onSystemReset }) => {
  ...
  // 1. Peces en tratamiento (Salud)
  // Nota: Necesitamos acceder a healthRecords que ahora se pasan al Dashboard
  // Como en v3.0 el Dashboard no recibía healthRecords, los detectaremos si el usuario los añade.
  // Por ahora, activaremos las alertas visuales si hay incidentes o parámetros críticos.
  ...
  const sickCount = (healthRecords || []).filter(h => h.status === 'treating').length;
