    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="grid grid-cols-2 gap-4">
        <Card className="bg-aqua-400/10 border-aqua-400/20 p-4 h-32 flex flex-col justify-between">
          <div className="flex items-center gap-3"><div className="w-8 h-8 rounded-lg bg-aqua-400/20 flex items-center justify-center text-aqua-400 font-white uppercase"><Fish size={20} /></div><span className="text-[10px] text-aqua-400 uppercase font-bold tracking-widest">STOCK TOTAL</span></div>
          <div className="flex items-end justify-between"><div className="flex flex-col"><span className="text-3xl font-display font-bold text-white">{totalStock}</span><span className="text-[8px] text-slate-500 uppercase font-bold mt-1 tracking-tighter">Ejemplares Vitales</span></div><div className="text-[9px] text-aqua-400/80 font-bold uppercase text-right opacity-80 font-black"><p>{totalMales} M</p><p>{totalFemales} H</p></div></div>
        </Card>
        <Card className={cn("p-4 h-32 flex flex-col justify-between transition-all border bg-red-500/10 border-red-500/20")}>
          <div className="flex items-center gap-3"><div className="w-8 h-8 rounded-lg bg-red-500/20 flex items-center justify-center text-red-500"><AlertTriangle size={20} /></div><span className="text-[10px] text-red-500 uppercase font-bold tracking-widest">ALERTAS / SUCESOS</span></div>
          <div className="flex items-end justify-between">
            <div className="flex flex-col"><span className="text-3xl font-display font-bold text-white">{activeIncidents.length}</span><span className="text-[9px] text-red-500/80 font-bold uppercase tracking-widest">ACTUAL</span></div>
            {activeIncidents.length === 0 && <span className="text-[10px] text-emerald-400 font-bold uppercase italic tracking-widest font-white">OK</span>}
          </div>
        </Card>
      </div>

      <Card className="p-5 bg-white/5 border border-white/10 relative overflow-hidden">
        <div className="flex items-center gap-2 mb-6 border-b border-white/5 pb-2 text-white"><Layers size={18} className="text-gold-400" /><h3 className="text-xs uppercase font-black tracking-widest">Inventario Detallado</h3></div>
        ...
        <div className="mt-6 pt-4 border-t border-white/5">
           {filteredCensus.filter(c => c.type === 'Alevines').map(item => (
             <div key={item.id} className="flex justify-between items-center bg-aqua-400/5 p-3 rounded-xl border border-aqua-400/10 animate-pulse">
               <div className="flex items-center gap-3 text-aqua-400"><Fish size={16} /><span className="text-[10px] font-bold uppercase tracking-widest">Alevines en Crecimiento</span></div>
      ...
      <Card className="p-5 bg-white/5 border border-white/5">
        <div className="flex justify-between items-center mb-6"><div className="flex items-center gap-3"><Droplets size={20} className="text-aqua-400" /><span className="text-[10px] font-bold text-white uppercase tracking-widest">Último Análisis Técnico</span></div><div className="text-[8px] text-slate-500 font-bold uppercase tracking-widest flex items-center gap-1.5"><Calendar size={12} />{latestParams ? new Date(latestParams.date).toLocaleString() : 'PENDIENTE'}</div></div>
        {latestParams ? (
          <div className="grid grid-cols-4 sm:grid-cols-7 gap-4">
            {[ {l:'pH',v:latestParams.ph,c:'text-aqua-400'}, {l:'T°',v:latestParams.temp,c:'text-white font-white'}, {l:'gH',v:latestParams.gh,c:'text-slate-400 font-white'}, {l:'kH',v:latestParams.kh,c:'text-slate-400 font-white'}, {l:'NH3',v:latestParams.nh3,c:'text-orange-500 font-white'}, {l:'NO2',v:latestParams.no2,c:'text-red-500 font-white'}, {l:'NO3',v:latestParams.no3,c:'text-orange-400 font-white'} ].map(m => (
