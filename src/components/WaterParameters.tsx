...
export const WaterParameters: React.FC<WaterParametersProps> = ({ waterParams = [], breedingProjects = [], onAdd, onUpdate }) => {
  ...
  return (
    <div className=\"space-y-6\">
      ...
      <div className=\"space-y-6\">
        {Object.entries(
          [...(waterParams || [])].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
            .reduce((acc, param) => {
              const week = getWeekRange(param.date).label;
          ...
          return (
            <div key={weekLabel} className=\"space-y-3\">
              ...
              >
                <div className={cn(\"w-1 h-4 bg-aqua-500 rounded-full transition-all\", !isExpanded && \"h-2 bg-slate-700\")} />
                <span className=\"text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 group-hover:text-aqua-400 transition-colors\">
                  {weekLabel}
                </span>
                <div className=\"h-px flex-1 bg-white/5 mx-2\" />
                <span className=\"text-[8px] font-bold text-slate-600\">{params.length} REGISTROS</span>
              </button>
