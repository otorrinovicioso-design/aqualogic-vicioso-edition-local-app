        ...
        {filteredData.map((item) => (
          <Card key={item.id} className="p-4 flex flex-col gap-4 bg-white/5 border border-white/10">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 rounded-xl bg-aqua-400/10 flex items-center justify-center text-aqua-400 transition-all">{item.type === 'Alevines' ? <Baby size={20} /> : <Fish size={20} />}</div>
